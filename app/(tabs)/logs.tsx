...
import { analyzeLogWithProvider } from "@/lib/ai-client";
...
function levelColor(level: LogLevel, _colors: any): string {
  switch (level) {
...
export default function LogsScreen() {
  ...
  const { connectionStatus, aiSettings } = useAppStore();
  ...
  const handleAiAnalyze = async (type: "error" | "analyze" | "optimize" | "custom") => {
    ...
    try {
      ...
      // Get the default provider and its config
      const provider = aiSettings.defaultProvider || 'builtin';
      const providerConfig = aiSettings.providers[provider];

      console.log('[Logs] AI Analysis:', {
        provider,
      ...
      if (provider !== 'builtin' && !providerConfig.apiKey) {
        throw new Error(`AI provider ${provider} requires API key but none is configured`);
      }

      console.log('[Logs] Calling analyzeLogWithProvider with:', {
        provider,
        hasApiKey: !!providerConfig.apiKey,
      ...
      const result = await analyzeLogWithProvider(
        {
          logType: type || 'general',
          content: prompt,
        },
        provider as any,
        providerConfig
      ...
      console.log('[Logs] AI analysis result:', {
      ...
      if (result.success && result.data) {
        console.log('[Logs] AI analysis succeeded');
        setAiResult({ summary: result.data, suggestions: [] });
      } else {
        console.error('[Logs] AI analysis failed:', result.error);
        const errorMsg = result.error || t('logs.analysisFailed');
        setAiResult({ summary: errorMsg, suggestions: [] });
      }
    } catch (err: any) {
  ...
  return (
    <ScreenContainer className="flex-1">
      ...
      {/* Log Output */}
      <View style={[styles.logContainer, { backgroundColor: "#0F1117" }]}>
        {!isConnected ? (
          ...
          <FlatList
            ...
            contentContainerStyle={[styles.logContent, { backgroundColor: "#0F1117" }]}
            onContentSizeChange={() =>
      ...
      {isConnected && (
        <View style={[styles.footer, { backgroundColor: "#161922", borderTopColor: colors.border }]}>
          <TouchableOpacity
            style={[
              styles.streamBtn,
      ...
      {/* AI Analysis Modal */}
          ...
          >
            <ScreenContainer edges={["top", "left", "right", "bottom"]} className="flex-1" containerClassName="bg-background">
