...
import { DeviceScreenPreview } from "@/components/device-screen-preview";
import { parseDeviceArchitecture, type DeviceArchitecture } from "@/lib/device-manager";
...
export default function DeviceScreen() {
  ...
  const [cpuArchitecture, setCpuArchitecture] = useState<string | undefined>(undefined);
  const [deviceArchitecture, setDeviceArchitecture] = useState<DeviceArchitecture>('unknown');
  const [deviceModel, setDeviceModel] = useState<'comma2' | 'comma3' | 'comma3x' | 'comma4' | 'unknown'>('unknown');
  ...
  const fetchDeviceInfo = useCallback(async () => {
    ...
    try {
      ...
      // Extract device architecture from the raw response
      let rawArch = raw.architecture || "";
      console.log('[DEBUG] Raw architecture from getDeviceInfo:', rawArch);
      console.log('[DEBUG] Full raw object keys:', Object.keys(raw));
      Object.entries(raw).forEach(([k, v]) => {
        console.log(`[DEBUG] raw.${k} = "${v}"`);
      });
      
      // If architecture is empty, try to infer from hostname or other sources
      if (!rawArch && raw.hostname) {
        console.log('[DEBUG] Architecture empty, trying to infer from hostname:', raw.hostname);
        rawArch = raw.hostname;
      }
      
      const parsed = parseDeviceArchitecture(rawArch || "unknown");
      console.log('[DEBUG] Parsed architecture:', parsed);
      setDeviceArchitecture(parsed);
      
      if (raw._error) {
        console.log('[DEBUG] SSH Error:', raw._error);
      }
  ...
  }, [setDeviceInfo, setDeviceArchitecture, setDebugInfo]);

  // Architecture is now fetched from getDeviceInfo() in fetchDeviceInfo()
  // No need for a separate fetchArchitecture() call

  const fetchDeviceModel = useCallback(async () => {
    if (!sshService.isConnected) return;
    try {
      const model = await sshService.getDeviceModel();
      setDeviceModel(model);
    } catch (err) {
      console.warn('Failed to fetch device model:', err);
  ...
  useEffect(() => {
    if (isConnected) {
      ...
      setCpuArchitecture(undefined);
      setDeviceArchitecture('unknown');
      setDeviceModel('unknown');
    }
    return () => { if (refreshInterval.current) clearInterval(refreshInterval.current); };
  }, [isConnected, fetchDeviceInfo, fetchDeviceModel]);
  ...
  return (
    <ScreenContainer>
      ...
      >
        ...
        {isConnected && (
          <>
            {/* Top row: Device Preview (left) + System Info (right) */}
            <View style={styles.topRow}>
              {/* Device Screen Preview - left side */}
              <DeviceScreenPreview
                architecture={deviceArchitecture}
                isConnected={isConnected}
              />

              {/* System Info Cards - right side, two small cards */}
              <View style={{ flex: 1, gap: 4 }}>
                {/* System Version Card */}
                <View style={[styles.smallInfoCard, { backgroundColor: colors.surface, borderColor: colors.border, paddingVertical: 6, paddingHorizontal: 8 }]}>
                  <Text style={{ fontSize: 8, color: colors.muted }}>Ver.</Text>
                ...
                {/* Platform Code Card */}
                <View style={[styles.smallInfoCard, { backgroundColor: colors.surface, borderColor: colors.border, paddingVertical: 6, paddingHorizontal: 8 }]}>
                  <Text style={{ fontSize: 8, color: colors.muted }}>Arch</Text>
                  <Text style={{ fontSize: 14, fontWeight: '700', color: deviceArchitecture === 'unknown' ? colors.muted : colors.foreground, marginTop: 2 }}>
                    {deviceArchitecture === 'unknown' ? '未知代号' : (() => {
                      ...
                      return archMap[deviceArchitecture] || deviceArchitecture.toUpperCase();
                    })()}
                  </Text>
                </View>
              </View>
            ...
            {/* Uptime row - full width */}
            {deviceInfo.uptime && (
              <View style={[styles.uptimeRow, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                  <Text style={{ fontSize: 18, color: colors.muted }}>⏱</Text>
                  <Text style={{ color: colors.muted, fontSize: 14 }}>{t("device.uptime")}</Text>
                </View>
                <Text style={{ color: colors.foreground, fontSize: 14, fontWeight: '500' }}>
                  {(() => {
                    const uptimeStr = deviceInfo.uptime || '';
                    const match = uptimeStr.match(/up\s+(\d+)\s+hours?,\s*(\d+)\s+minutes?/i);
                    if (match) {
            ...
            <TouchableOpacity
              ...
              style={[styles.debugToggle, { borderColor: colors.border, backgroundColor: colors.surface }]}
            >
            ...
            {showDebug && Object.keys(debugInfo).length > 0 && (
              <View style={[styles.debugBox, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Text style={{ color: colors.foreground, fontSize: 12, fontWeight: '600', marginBottom: 8 }}>SSH 原始数据：</Text>
                {Object.entries(debugInfo).map(([key, value]) => (
                  <Text key={key} style={{ color: colors.muted, fontSize: 10, marginBottom: 4, fontFamily: 'monospace' }}>
...
const styles = StyleSheet.create({
  ...
  compactValue: { fontSize: 13, fontWeight: "600", marginTop: 2 },
  infoRow: {
    flexDirection: "row",
    alignItems: "center",
  ...
  // Top row layout: device preview (left) + system info (right)
  topRow: {
    flexDirection: "row",
    gap: 12,
    marginBottom: 16,
